...
func TestPathManagerIntentionalMigration(t *testing.T) {
	...
	require.False(t, shouldSwitch)
	require.Equal(t, []protocol.ConnectionID{connIDs[0], connIDs[1]}, retiredConnIDs)
}
...
func TestPathManagerMultipleProbes(t *testing.T) {
	...
	require.Equal(t, connIDs[0], connID)
	require.Len(t, frames, 2)
	require.Equal(t, &wire.PathResponseFrame{Data: [8]byte{1, 2, 3, 4, 5, 6, 7, 8}}, frames[1].Frame)
...
func TestPathManagerLimits(t *testing.T) {
	...
	// Losing a challenge retains its CID and the bounded path record.
	f1[0].Handler.OnLost(f1[0].Frame)

	// No new path can be allocated until an existing record expires.
	connID, frames, _ = pm.HandlePacket(&net.UDPAddr{IP: net.IPv4(1, 2, 3, 4), Port: 4000}, now, nil, true)
	require.Zero(t, connID)
	require.Empty(t, frames)
	// Existing paths still answer challenges even when the table is full.
	_, frames, _ = pm.HandlePacket(&net.UDPAddr{IP: net.IPv4(1, 2, 3, 4), Port: 3001}, now, &wire.PathChallengeFrame{}, false)
	require.Len(t, frames, 2)
	now = now.Add(pathTimeout)
	connID, frames, _ = pm.HandlePacket(&net.UDPAddr{IP: net.IPv4(1, 2, 3, 4), Port: 4000}, now, nil, true)
	require.NotEmpty(t, frames)
	require.Equal(t, connIDs[connIDIndex], connID)
	require.Len(t, pm.paths, maxPaths)
}
