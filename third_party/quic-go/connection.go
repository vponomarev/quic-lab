...
type receivedPacket struct {
	transport *Transport // receiving path, including paths not yet selected for sending
	buffer    *packetBuffer
...
type Conn struct {
	...
	retireTransport      chan retireTransportRequest
...
func (c *Conn) preSetup() {
	...
	c.retireTransport = make(chan retireTransportRequest)
	...
	for {
		...
		if !shouldProceedImmediately {
			...
			case <-c.closeChan:
				break runLoop
			case <-c.timer.C:
			case req := <-c.retireTransport:
				if addrsEqual(c.conn.LocalAddr(), req.transport.conn.LocalAddr()) {
					req.result <- errors.New("transport is still active")
				} else {
					c.connIDGenerator.RemoveConnRunner((*packetHandlerMap)(req.transport))
					req.result <- nil
				}
...
func (c *Conn) handleShortHeaderPacket(
	...
	if c.perspective == protocol.PerspectiveClient {
		if pathChallenge != nil && p.transport != nil {
			// RFC 9000 section 8.2.2: respond on the path that carried the
			// challenge, even before the application switches to that path.
			cid := c.connIDManager.Get()
			if pm := c.pathManagerOutgoing.Load(); pm != nil {
				if pathCID, ok := pm.connIDForTransport(p.transport); ok {
					cid = pathCID
				}
			}
			probe, buf, err := c.packer.PackPathProbePacket(cid, []ackhandler.Frame{{Frame: &wire.PathResponseFrame{Data: pathChallenge.Data}, Handler: pathResponseHandler{}}}, c.version)
			if err != nil {
				return false, err
			}
			...
			buf.Release()
			if err != nil {
				// A queued challenge may belong to a retired / failed standby.
				// Failure on that path must not tear down the working connection.
				c.logger.Debugf("could not send PATH_RESPONSE on receiving path: %s", err)
			}
		}
		return true, nil
	}
...
func (c *Conn) handlePathChallengeFrame(f *wire.PathChallengeFrame) {
	// The packet handler responds using the receiving transport.
}
