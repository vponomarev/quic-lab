...
func (h *connIDManager) add(f *wire.NewConnectionIDFrame) error {
	...
	inUse := f.SequenceNumber == h.activeSequenceNumber
	for _, entry := range h.pathProbing {
		inUse = inUse || entry.SequenceNumber == f.SequenceNumber
	}
	if !inUse && (f.SequenceNumber < max(h.activeSequenceNumber, h.highestProbingID) || f.SequenceNumber < h.highestRetired) {
...
func (h *connIDManager) addConnectionID(seq uint64, connID protocol.ConnectionID, resetToken protocol.StatelessResetToken) error {
	// A retransmitted NEW_CONNECTION_ID must not put an allocated probing CID
	// back into the free queue. Otherwise two paths share it and retiring one
	// silently invalidates the other at the peer.
	for _, entry := range h.pathProbing {
		if entry.SequenceNumber == seq {
			if entry.ConnectionID != connID || entry.StatelessResetToken != resetToken {
				return fmt.Errorf("received conflicting probing connection ID for sequence number %d", seq)
			}
			return nil
		}
	}
