...
internal class QuicSession(
    ...
    init {
        watch(WIFI); watch(CELLULAR)
        worker.scheduleWithFixedDelay({
            if (!closed) try { tick() } catch (e: Exception) { event("controller_error", e.toString()) }
    ...
    fun setAutomatic(enabled: Boolean) = submit {
        automatic = enabled
        event("auto_mode", if (enabled) "Автомиграция включена; переподключение выключено" else "Автомиграция выключена")
        if (enabled && failedNetwork != null) recover("Автомиграция включена")
    }

    fun startOrMigrate(kind: Int, endpoint: String, name: String, pin: String, interval: Long) = submit {
        try {
            ...
            if (alive) {
                ...
                val current = Mobile.newClient(object : EventSink {
                    override fun onEvent(eventJSON: String) {
                        ...
                        if (e.optString("event") == "echo") {
                            lastEchoAt = now()
                            smoothedRTT = smoothedRTT * 0.875 + e.optDouble("rtt_ms", 100.0) * 0.125
                        }
                        if (e.optString("event") == "standby_ready") standbyRTT = e.optDouble("probe_ms")
                        output(e)
                        ...
                        if (e.optString("event") == "path_unavailable") submit {
                            if (token == epoch && alive && e.optString("local") == client?.localAddress()) {
                ...
                current.start(resolved.address, name.ifBlank { resolved.hostname }, pin, interval, binder(network))
                alive = true
                intervalMS = interval
                lastEchoAt = now()
                lastSwitchAt = now()
    ...
    private fun recover(reason: String) {
        ...
        val candidate = listOf(WIFI, CELLULAR).mapNotNull { kind ->
            networks[kind]?.takeIf { it != activeNetwork && (retryAt[it] ?: 0L) <= now() }?.let { kind to it }
        }.firstOrNull()
        if (candidate == null) {
            if (failedNetwork != previous) event("waiting_network", "$reason. Ожидание доступного пути без нового соединения.")
    ...
    private fun tick() {
        ...
        val time = now()
        if (activeNetwork in networks.values && lastEchoAt > lastAttemptAt && time - lastEchoAt < maxOf(150L, intervalMS * 2)) {
            failedNetwork = null
            if (time - lastSwitchAt > 30000) activeNetwork?.let { failures.remove(it); retryAt.remove(it) }
        }
        ...
        val reserve = networks.entries.firstOrNull { it.value != activeNetwork && (retryAt[it.value] ?: 0L) <= time } ?: return
        ...
        try {
            ...
            if (activeKind == CELLULAR && reserve.key == WIFI && reserve.value in validated &&
                policy.canPreferWifi(now() - (readySince[reserve.value] ?: now()), now() - lastSwitchAt) &&
                standbyRTT < maxOf(150.0, smoothedRTT * 1.25)) {
                migrate(WIFI, reserve.value, "Wi-Fi устойчиво отвечает")
            } else if (smoothedRTT > 600.0 && standbyRTT * 2 < smoothedRTT &&
                now() - lastSwitchAt > 8000 && now() - (readySince[reserve.value] ?: now()) > 3000) {
    ...
    private fun watch(kind: Int) {
        val callback = object : ConnectivityManager.NetworkCallback() {
            ...
            override fun onCapabilitiesChanged(network: Network, caps: NetworkCapabilities) = submit {
                val ready = caps.hasCapability(NetworkCapabilities.NET_CAPABILITY_VALIDATED)
                validation(network, kind, ready)
                val newlyReady = if (ready) validated.add(network) else { validated.remove(network); false }
            ...
            override fun onLosing(network: Network, maxMsToLive: Int) = submit {
                if (network == activeNetwork && preparedNetwork != null && now() - preparedAt < 3000) {
    ...
    fun stop() = submit { stopInternal(); event("stopped", "Опыт остановлен") }
    ...
    private fun binder(network: Network) = object : SocketBinder {
