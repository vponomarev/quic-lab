...
{{if .Echo}}<p class="muted">Откройте QUIC Lab на телефоне и нажмите «Сканировать QR». Вход для echo не нужен.</p><section class="card qr"><img src="{{.QR}}" alt="QR настроек echo"><p><code>{{.Endpoint}}</code></p><small>TLS: {{.Hostname}}</small>{{if .APKSize}}<p><a class="download" href="{{.Base}}download/quic-lab.apk" download>Скачать QUIC Lab для Android · {{.APKSize}}</a></p><small>Android 12+ · ARM64 / x86-64. Установите APK, затем отсканируйте QR-код в приложении.</small><details class="download-qr"><summary>Показать QR для скачивания</summary><p>Сканируйте камерой телефона, чтобы скачать APK.</p><img src="{{.APKQR}}" alt="QR для скачивания APK QUIC Lab"></details>{{end}}</section>{{end}}
